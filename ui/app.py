...
def confidence_label(conf: float) -> str:
    if conf >= 0.90:
        return f"{conf * 100:.1f}% — Deployment Corroborated"
    elif conf >= 0.75:
        return f"{conf * 100:.1f}% — Telemetry Evidence"
    elif conf >= 0.50:
        return f"{conf * 100:.1f}% — Multiple Causes Plausible"
    else:
        return f"{conf * 100:.1f}% — Speculative"
...
with st.sidebar:
    ...
    st.markdown("### Confidence Rubric")
    st.markdown("""
| Score | Evidence Quality |
|---|---|
| 0.90–1.00 | Deployment directly precedes failure |
| 0.75–0.89 | Strong telemetry, no deployment anchor |
| 0.50–0.74 | Multiple causes plausible |
| < 0.50 | Speculative — execution blocked |
""")
    ...
    try:
        ...
        with st.expander(f"View History ({len(audit_data)} events)"):
            for entry in reversed(audit_data[-5:]):
                ...
                st.caption(f"Confidence: {confidence_label(conf)}")
                st.divider()
...
""")
        ...
        if st.session_state.get('rca_complete', False):
            ...
            col_m2.metric("Confidence", confidence_label(conf))
...
with tab3:
    if st.session_state.get('rca_complete', False):
        ...
        if confidence < 0.50:
            st.error(
                f"❌ **SPECULATIVE DIAGNOSIS:** Score {confidence * 100:.0f}% — "
                f"Insufficient evidence. Remediation blocked. Manual investigation required."
            )
        elif confidence < 0.75:
            st.error(
                f"❌ **CONFIDENCE THRESHOLD FAILED:** Score {confidence * 100:.0f}% is below the 75% minimum. "
